local Keys = {
  ...
  function OpenVehicleSpawnerMenu(type, station, part, partNum)
	  ...
	  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		  ...
		  elseif data.current.action == 'police_vehicle' then
			  ...
			  local garage_admin = {}
			  ESX.TriggerServerCallback('esx_adminjob:getVehicleFromadmin', function(jobVehicles)
				  if #jobVehicles > 0 then
					  for k,v in ipairs(jobVehicles) do
						  local props = json.decode(v.vehicle)
						  local vehicleName = GetLabelText(GetDisplayNameFromVehicleModel(props.model))
						  ...
						  if v.admin_by then
							  label = label .. ('โดย: %s'):format(v.admin_by).." ".. v.time
						  else
							  label = label .. ('โดย: %s'):format("Unknow").." ".. v.time
						  end
  
						  table.insert(garage_admin, {
							  label = label,
							  value = k
							  -- stored = v.stored,
							  -- value = props
					  ...
					  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage1', {
						  ...
						  if foundSpawn then
							  ...
							  ESX.Game.SpawnVehicle(props.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
								  ...
								  TriggerServerEvent('esx_adminjob:setVehicleFromadmin', props.plate)
								  exports.nc_notify:PushNotification({
		  ...
		  ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm',
		  {
			  ...
			  if data2.current.value == 'yes' then
				  ...
				  ESX.TriggerServerCallback('esx_adminjob:buyJobVehicle', function (bought)
					  if bought then
						exports.nc_notify:PushNotification({
							title = ' คุณได้ซื้อรถ ' .. data.current.name, ESX.Math.GroupDigits(data.current.price),
							description = message,
							color = '#94dcfc',
							icon = 'message',
							direction = 'right',
		...
		--	  {label = "เบิกใบ Pound",               value = 'xenon_pound'}
			  ...
			  'default', GetCurrentResourceName(), 'citizen_interaction',
			  {
				  ...
				  if closestPlayer ~= -1 and closestDistance <= 3.0 then
					  ...
					  elseif action == 'handcuff' then
						  ...
						  TriggerServerEvent('esx_adminjob:handcuff', GetPlayerServerId(closestPlayer), playerheading, playerCoords, playerlocation)
					  elseif action == 'unhandcuff' then
						  playerheading = GetEntityHeading(GetPlayerPed(-1))
						  playerlocation = GetEntityForwardVector(PlayerPedId())
						  playerCoords = GetEntityCoords(GetPlayerPed(-1))
						  TriggerServerEvent('esx_adminjob:unhandcuff', GetPlayerServerId(closestPlayer), playerheading, playerCoords, playerlocation)
		  ...
		  elseif data.current.value == 'vehicle_interaction' then
			  ...
			  if DoesEntityExist(vehicle) then
				  ...
				  table.insert(elements, {label = "ยึดรถ",		value = 'impound_admin'})
			  end
			  ...
			  'default', GetCurrentResourceName(), 'vehicle_interaction',
			  {
				  ...
				  elseif DoesEntityExist(vehicle) then
					  ...
					  elseif action == 'hijack_vehicle' then
						  if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
							...
							--   })
							  ...
							  TriggerServerEvent('azael_dc-serverlogs:insertData', 'adminHijack', content, GetPlayerServerId(PlayerId()), '^5')
						  end
					  ...
					  elseif action == 'impound_admin' then
		  ...
		  -- elseif data.current.value == 'squad' then
			  -- TriggerEvent("meeta_admin:squadMenu")
  ...
  function CreateBillingPrice(player, value)
	  ...
	  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'dialog_createbill_price',
	  {
		  ...
		  if tonumber(data2.value) then
			  TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_admin',"admin: "..value, tonumber(data2.value))  --society_admin
			  menu2.close()
			  exports.nc_notify:PushNotification({
	  ...
	  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	  {
		  ...
		  if data.value == nil then
			exports.nc_notify:PushNotification({
			  ...
			  ESX.TriggerServerCallback('esx_adminjob:getVehicleByName', function(result, found)
				  if found then
	  ...
	  ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	  {
		  ...
		  if data.value == nil then
			exports.nc_notify:PushNotification({
			  ...
			  ESX.TriggerServerCallback('esx_adminjob:getHouseByName', function(result, found)
				  if found then
  ...
  AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	  if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'admin' and PlayerData.job.name == dispatchNumber then
  ...
  AddEventHandler('esx_adminjob:hasEnteredEntityZone', function(entity)
	  ...
	  if PlayerData.job ~= nil and PlayerData.job.name == 'admin' and IsPedOnFoot(playerPed) then
  ...
  RegisterNetEvent('esx_adminjob:getarrested')
  AddEventHandler('esx_adminjob:getarrested', function(playerheading, playercoords, playerlocation)
	  ...
	  ESX.Streaming.RequestAnimDict("mp_arrest_paired", function()
		  ...
		  Citizen.Wait(3060)
		  TriggerEvent("esx_adminjob:handcuff")
	  end)
	  
  end)
  
  RegisterNetEvent('esx_adminjob:douncuffing')
  AddEventHandler('esx_adminjob:douncuffing', function()
	  Citizen.Wait(250)
  ...
  RegisterNetEvent('esx_adminjob:getuncuffed')
  AddEventHandler('esx_adminjob:getuncuffed', function(playerheading, playercoords, playerlocation)
	  local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	  SetEntityCoords(GetPlayerPed(-1), x, y, z)
	  SetEntityHeading(GetPlayerPed(-1), playerheading)
	  Citizen.Wait(250)
  ...
  RegisterNetEvent('esx_adminjob:doarrested')
  AddEventHandler('esx_adminjob:doarrested', function()
	  Citizen.Wait(250)
	  ESX.Streaming.RequestAnimDict("mp_arrest_paired", function()
  ...
  Citizen.CreateThread(function()
	  while true do
		  ...
		  if CurrentAction ~= nil then
			  ...
			  if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'admin' then
		  ...
		  if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'admin' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'admin_actions') then
			  if Config.MaxInService == -1 then
  ...
  function ImpoundVehicle(vehicle)
	  ...
	  TriggerServerEvent('azael_dc-serverlogs:insertData', 'adminImPound', content, GetPlayerServerId(PlayerId()), '^5')
  ...
  function ImpoundVehicleadmin(vehicle)
	  local plate = ESX.Math.Trim(GetVehicleNumberPlateText(vehicle))
	  local content = '' .. GetPlayerName(PlayerId()) .. ' ยึดรถ ทะเบียน ' .. plate .. ' ไปยังพาวท์'
	  TriggerServerEvent('azael_dc-serverlogs:insertData', 'adminImPound', content, GetPlayerServerId(PlayerId()), '^3')
  
	  TriggerServerEvent('esx_adminjob:updateVehicleFromadmin', plate)
	  ESX.Game.DeleteVehicle(vehicle) 
	  exports.nc_notify:PushNotification({
		title = ' พาวน์รถเรียบร้อยแล้ว ',
		description = message,
  ...
  AddEventHandler("esx_adminjob:closeall", function()
	  ESX.UI.Menu.CloseAll()
  end) 
  
  RegisterCommand('Closeall', function()
	  TriggerEvent('esx_adminjob:closeall')
  ...
  System = {
	  ['esx_adminjob:drag'] = function(copID)	
		  if IsHandcuffed then
				  playerPed = PlayerPedId()	
			  if Drag == false then		
